...
Accidents_2017 %>%
  ...
  summarise(Count = n() ) %>%
  ungroup() %>%
  ggplot(mapping = aes(x = Casualty_Class)) +
  geom_bar() +
  facet_wrap(~Casualty_Class)+
  ggtitle("Number of Casualties Per Age For Each Casualty Class") +
  labs(fill = "Casualty Class") +
  ylab("Number of Casualties") 

Accidents_2017 %>%
  group_by(Casualty_Class, Gender,Age) %>%
  summarise(Count = n() )
