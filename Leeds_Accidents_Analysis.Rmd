...
# Covariance (Age against Count, Classified by Gender)
...
#### For females as the age increase the count increases. The count peaks around the mid-20s,then the count declines gradually. The males' graph has a similar shape as females'. The only differences is that the males' graph has a higher peak and the drops sharper. 
...
#### Most serious and slight accidents happen between 10 am and 5.30 pm. While most fatal accidents occur between 4 pm and 8 pm. Serious accidents have the widest inter-quartile range while fatal accidents have the smallest inter=quartile range.   
...
```{r}
# Covariance (Age against Count, Classified by Casualty Class)
...
```
#### When the casualty is a driver or rider, as the age increase till the mid-30s the count rockects,then the count falls rapidly. The plots for pedestrains and passengers are very similar,the only disparity is that the passengers have a higher peak. It should be highlighted that the number of casualties between the age 20 till mid-50s is awfully high when the casualty is a driver or rider compared to other casualty classes. Moreover, in the pedestrains' and passengers plots, teenagers have the higher number of casaulties than other age groups. Lastly, the number of infants and children casualties is dreadfully high when they are passengers in comparison to other casualty classes.
